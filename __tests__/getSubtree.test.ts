...
test("Confirm subtree without a match", () => {
  expect(getSubtree(tree1, { testFn: (x) => x.id === 2 })).toBe(null)
  expect(getSubtree(tree6, { testFn: (x) => x.id === 999 })).toBe(null)
  expect(getSubtree(tree2, { testFn: (x) => x.foo == "nonexistent" })).toBe(null)
})
